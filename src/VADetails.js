...
const VADetails = () => {
  ...
  useEffect(() => {
    ...
    const currentVa = savedVas[id];
    if (currentVa) {
      setVa(currentVa);
      setPaymentMethod(currentVa.paymentMethod || '');
      setPaymentDetails(currentVa.paymentDetails || '');
      setLastPayment(currentVa.lastPayment || '2023-07-01');
    }
  }, [id]);
  ...
  const handleCreateInvoice = () => {
    ...
    const invoiceContent = `
      ...
      Client: Monsieur/Madame ${va.firstName} ${va.lastName}
      ...
      <Grid container justifyContent="space-between" alignItems="center" sx={{ mt: 1, mb: 1 }}>
        <Grid item>
          <Typography variant="h4" sx={{ color: '#fff' }}>
            VA Details - {va.firstName} {va.lastName}
          </Typography>
        </Grid>
        <Grid item>
        ...
        <Grid item xs={12} sm={6} md={2.4}>
          <Paper elevation={3} sx={{ backgroundColor: '#3d3d3d', p: 2, textAlign: 'center', height: '70%' }}>
            ...
            <Typography variant="h5" sx={{ color: '#fff' }}>
              {va.paymentPlan === 'percentage' ? `Paid per %: ${va.amount}%` : `Salary: $${va.amount}`}
            </Typography>
          </Paper>
        </Grid>
      </Grid>
