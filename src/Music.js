  ...
  return (
    <Container>
      <Box sx={{ display: 'flex', alignItems: 'center', mb: 3 }}>
        ...
        <MusicNoteIcon fontSize="large" sx={{ color: '#fff' }} />
      </Box>
      <Box sx={{ mb: 3 }}>
        <Select
          value={model}
          ...
          <List>
            {accounts.map((account, index) => (
              <ListItem key={index} sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: '#fff', fontWeight: 'bold' }}>
                <ListItemText
                  primary={
                    <Link href={`https://www.tiktok.com/@${account.username}`} target="_blank" rel="noopener noreferrer" sx={{ color: 'blue', fontWeight: 'bold', textDecoration: 'none' }}>
                      {account.username}
                    </Link>
                  }
